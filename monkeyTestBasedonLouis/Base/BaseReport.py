...
class OperateReport:
    ...
    def plot(self, worksheet, types, lenData, name):
        ...
        title = ""
        if types == "cpu":
            values = "="+name+"!$A$2:$A$" + str(lenData + 1)
            row = 'A' + str(lenData)
            title = "cpu使用率"
        elif types == "men":
            values = "="+name+"!$B$2:$B$" + str(lenData + 1)
            row = 'B' + str(lenData)
            title = "内存使用MB"
        elif types == "fps":
            values = "=" + name + "!$C$2:$C$" + str(lenData + 1)
            row = 'C' + str(lenData)
            title = "fps使用情况"
        elif types == "battery":
            values = "="+name+"!$D$2:$D$" + str(lenData + 1)
            row = 'D' + str(lenData)
            title = "电池剩余%"
        elif types == "flowUp":
            values = "="+name+"!$E$2:$E$" + str(lenData + 1)
            row = 'E' + str(lenData)
            title = "上行流量KB"
        elif types == "flowDown":
            values = "="+name+"!$F$2:$F$" + str(lenData + 1)
            row = 'F' + str(lenData)
            title = "下行流量KB"
...
def _write_center(worksheet, cl, data, wd):
    ...
    if str(type(data)) == "<class 'dict'>":
        return worksheet.write(cl, str(data), get_format_center(wd))
    else:
